...
md"To simplify, we will say that the escape velocity is:

$v_{e}=\sqrt{2gr}$

where r approximate as the radius of the planet and g the constant of gravity at the surface. Suppose that we remember from school that the velocity of scape from Earth is $11\frac{km}{s}$ and that the radius of Mars if half of the earth's. "

# ╔═╡ b92e1dfa-1acb-11eb-3aae-a90766806c63
md"
We remember that the gravity of Earth at its surface is $9.8\frac{m}{s^2}$, so all we need to estimate is the gravity of the planet Mars at its surface. So  and your partner decide to make an experiment and gather some data. But what exactly do you need to measure? Let's see.
"

# ╔═╡ b54dec82-1883-11eb-0a61-313724e1a335
md"You are going to throw stones, so we are going to explain a bit the equations regarding the experiment. The topic we need to revisit is the topic of Proyectile Motion. "

# ╔═╡ 424b96de-1884-11eb-06fe-032b13ea20eb
md" ## Proyectile Motion"

# ╔═╡ 4aa3534c-1884-11eb-0c30-e387db85affe
md" Gravity pulls us down to Earth, in this case to Mars. Phisycally this means that we have and acceletation, recalling the newton equation:

$\overrightarrow{F} = m * \overrightarrow{a}$
 where *m* is the mass of the object, $\overrightarrow{F}$ is the force(it's what make us fall) and $\overrightarrow{a}$ is the acceleration, in our case is what we call gravity of $\overrightarrow{g}$. The arrow $\overrightarrow{}$ over the letter means that the quantity have a direction in space, in our case, gravity is pointing to the center of the Earth, or Mars.

How can we derive the motion of the stones with that equation?

In the figure below we show an sketch of the problem: We have the 2 axis, *x* and *y*, the *x* normally goes parallel to the ground and the *y* axis is perpendicular pointing to the sky. We also draw the initial velocity $v_0$ of the proyectile, and the angle $\theta$ with respect to the ground. Also it's important to notice that the gravity points in the opposite direction of the *y* axis."

# ╔═╡ 677b95c2-1888-11eb-18be-0de8ca145819
md" But what are the trayectory of the proyectile, meaning how the coordinates *x* and *y* evolve with time. 
...
md"where $t$ is the time at which we want to know the coordinates. What the equations say? If we see the evolution of the projectile in the *x* axis only, it follows a stright line (until it hits the ground) and in the *y* axis the movement follows a parabola, but how we interpret that? 

We can imagine what happens if we trow a stone to the sky: the stone starts to go up and then, at some point, the stone reach the highest position it can go and then starts to go down. How does the velocity evolve in this trajectory? Since the begining, velocity starts decreasing until it has the value of 0 at the highest point, where the stone stops for a moment, then it chages its direction and start to increase again, pointing towards the ground. Ploting the evolution of the height of the stone we obtain the plot shown below.  We see that at the begining the stone starts to go up 
fast and then it slows down. We see that for each value of *y* there are 2 values of *t* that satisfies the equation, thats because the stone pass twice for each point, except for the highest value of *y*."
...
md"So, in the example below, we have θ=90°, so sin(θ)=1 and the trajectory in *y* becomes:"
...
md" It's worth to notice that at each value *y* of the trajectory, the velocity could have 2 values, just differing in its sign, meaning the direction, but with the same magnitude. So keep in mind that when you trow and object, when it'll return the velocity will be the same with the one you trow it."

# ╔═╡ a26eaa4c-1938-11eb-1e8c-a1e95af7fe04
md"Now that we have the equations, how do we set the experiment and what do we need to measure? 
The set up will be like this:
- One person will be trowing stones with an angle.
- The other person will be a bit far, measuring the time since the other trow the stone and it hits the ground and the distance Δx travelled.
- Also, for the first iteration of the experiment, suppose we only keep the measurements with and initial angle θ~45°. 
"
...
md"You have measured then the 5 points of Δx and t shown below:"
...
Using the equations of the trajectory, when the stone hits the ground y(t) = 0 (negleting the initial height with respect to the maximum height), so finding the other then the initial point that fulfill this, we find that:
...
md" So, solving for $v_{0}$ we have:

$v_{0}=\frac{Δx}{t_{f}cos(θ)}$
"

# ╔═╡ 8e38f6a6-194f-11eb-29a2-ad835ecc8580
md"Then replacing it in the equation of $t_{f}$ and solving for *g* we obtain:

$Δx=\frac{g*t_{f}^2}{2*tg(θ)}$
"

# ╔═╡ 1048b23c-1951-11eb-0040-0d17e5c05670
md"The model we are goint to propose is a linear regresion. a linear equation has the form:
...
So, what this says is that each point of the regresion is draw of a gaussian distribution with its center correnponding with the point of the line, as shown in the plot below."

# ╔═╡ f64badec-1a12-11eb-19a6-8db5a0f7de03
begin 
	im_4 = load("./images/sketch_4.png")
end
...
md"Where *g* will have a distribution we propose. The first distribution we are going to propose is a Uniform distribution for g between the values of 0 and 10"

# ╔═╡ f5623f04-1a17-11eb-1d92-4ff256b15332
begin
plot(Uniform(0,10),xlim=(-1,11), ylim=(0,0.15), legend=false, fill=(0, .5,:lightblue))
...
md"We can propose a Gaussian distribution instead of a uniforn distribution for g, like the one shown below, with a mean of 5 and a variance of 2, and let the model update its beliefs with the points we have."
...
md" We define then the model with the gaussian distribution:"
...
md"## Optimizing throwing angle"

# ╔═╡ f816003a-1a23-11eb-2124-e59991aa0e5e
md"Now that we have a understanding of the equations and the problem in general, we are going to add some difficulties. Suppose that the device employed to measure the angle has an error of 15° no matter the angle. We want to know what are the most convenient angle to measure or if it doesn't matter."

# ╔═╡ 730d6e62-1a25-11eb-09cd-4133b7f6c59a
md"To do the analysis we need to see how is the impact of the angle in the computation of g, so solving the equation for g we have:

$g = \frac{2*tg(\theta)*\Delta x}{t^{2}_f}$
"

# ╔═╡ 5e96b0fc-1a27-11eb-3941-e914415aad76
md"So, we know that $\Delta \theta = 15°$, so now plotting the function $sec^{2}(\theta)$, which is proportional to the error of the gravity as function of θ:"

# ╔═╡ c32ae204-1ab5-11eb-0600-ab56ad81dad2
md"We see that the higher the angle, the higher the error, is it true? 

We can plot then tangent of θ as with and error of 15° and see how what is the maximum and minimun value:"
...
plot(angles, er .* 100 ./ μ, xlim=(5,70), ylim=(0,200), color="lightblue", legend=false)
ylabel!("Δtan(θ)/θ")
xlabel!("θ [deg]")
...
md"So, now we see that the lowest percentual error is obtained when we work in angles near 45°, so we are good to go and use the data we measured adding the error in the angle."
...
md"Now that we have calculated the gravity, we are going to calculate the escape velocity. What data we have? we know that:

$R_{Earth}\approxeq 2R_{Mars}$
$g_{Earth}\approxeq 9.8$

and we have a distribution of the plausible values of $g_{Mars}$.

So, replacing in the equation of the escape velocity:

$\frac{v_{Mars}}{v_{Earth}} =\sqrt{\frac{g_{Mars}*R_{Mars}}{g_{Earth}*R_{Earth}}}$

so,

$\frac{v_{Mars}}{11} =\sqrt{\frac{g_{Mars}*2*R_{Mars}}{9.8*R_{Mars}}}$

$v_{Mars} =11 * \sqrt{\frac{g_{Mars}*R_{Mars}}{9.8*2*R_{Mars}}}$
"
...
md"Finally, we obtained the escape velocity and we can scape from Mars."

# ╔═╡ 03b6d616-1ab6-11eb-08ef-b52a1b3ce7ea
tan(10)
